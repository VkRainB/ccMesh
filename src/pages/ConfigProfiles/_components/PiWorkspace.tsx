...
import {
  ...
  type PiConfigPaths,
  type PiProviderData,
  type PiWorkspaceState,
} from "@/services/modules/pi_config";
...
import { ThinkingLevelMapEditor } from "./_piOmpCommon/ThinkingLevelMapEditor";
import { CostTiersEditor } from "./_piOmpCommon/CostTiersEditor";
import { SamplingParamsEditor } from "./_piOmpCommon/SamplingParamsEditor";
...
function piFileTabs(paths: PiConfigPaths | undefined) {
  return [
    { value: "provider" as const, label: "Profile" },
    { value: "models" as const, label: fileBaseName(paths?.modelsPath, "models.json") },
    { value: "settings" as const, label: fileBaseName(paths?.settingsPath, "settings.json") },
  ];
}

const EMPTY_FIELDS: PiProviderFields = {
  name: "",
...
export function PiWorkspace() {
  ...
  const workspaceQuery = useQuery({
    queryKey: PI_QUERY_KEYS.workspace(),
    queryFn: () => piConfigApi.sync(),
  });
  ...
  const [fields, setFields] = useState<PiProviderFields>(EMPTY_FIELDS);
  ...
  const updateWorkspaceCache = (nextProviders: PiOmpProviderMeta[]) => {
    queryClient.setQueryData<PiWorkspaceState>(PI_QUERY_KEYS.workspace(), (oldWorkspace) => {
  ...
  const startNew = () => {
    ...
    const initialFields: PiProviderFields = {
      ...EMPTY_FIELDS,
      baseUrl: gateway,
      models: [emptyPiModel()],
    };
    ...
    setProviderText(formatProviderJson(applyFieldsToPiProvider({}, initialFields)));
    setRightTab("provider");
    setRightEditable(false);
    setShowKey(false);
    setDirty(false);
  ...
  const loadProvider = async (nextProviderId: string) => {
    try {
      const providerData: PiProviderData = await piConfigApi.getProvider(nextProviderId);
      const parsedFields = parsePiProviderFields(providerData.providerJson, gateway);
  ...
  const updateField = (patch: Partial<PiProviderFields>) => {
    setDirty(true);
    setFields((current) => {
      const next = { ...current, ...patch };
      try {
        const base = parseProviderJsonText(providerText);
        setProviderText(formatProviderJson(applyFieldsToPiProvider(base, next)));
      } catch {
  ...
  const onProviderTextChange = (text: string) => {
    ...
    try {
      setFields(parsePiProviderFields(parseProviderJsonText(text), gateway));
    } catch {
      // 非法 JSON 保留输入
    }
  };
  ...
  const addModel = () => {
    const nextModels = addPiModel(fields.models);
    updateField({ models: nextModels });
    setExpandedModel(nextModels.length - 1);
  };

  const updateModel = (index: number, patch: Partial<PiModelFields>) => {
    updateField({ models: updatePiModel(fields.models, index, patch) });
  };

  const removeModel = (index: number) => {
    updateField({ models: removePiModel(fields.models, index) });
  ...
  const toggleInput = (index: number, inputType: "text" | "image", checked: boolean) => {
    updateField({ models: togglePiModelInput(fields.models, index, inputType, checked) });
  };
  ...
  const saveCurrentProvider = async () => {
    ...
    return piConfigApi.saveProvider({
  ...
  const applyMutation = useMutation({
    mutationFn: async () => {
      ...
      const result = await piConfigApi.apply({
  ...
  const deleteMutation = useMutation({
    mutationFn: (deleteId: string) => piConfigApi.deleteProvider(deleteId),
    onSuccess: (workspace, deleteId) => {
      const nextProviders = sortProviders(workspace.providers);
      setProviders(nextProviders);
      setDefaultProvider(workspace.defaultSelection.provider ?? "");
      setDefaultModel(workspace.defaultSelection.model ?? "");
      queryClient.setQueryData(PI_QUERY_KEYS.workspace(), workspace);
  ...
  const renameMutation = useMutation({
    mutationFn: (args: { oldId: string; newId: string }) =>
      piConfigApi.renameProvider(args.oldId, args.newId),
  ...
  const fileTabs = useMemo(() => piFileTabs(workspaceQuery.data?.paths), [workspaceQuery.data?.paths]);
  ...
  const currentSelector = defaultProvider && defaultModel ? `${defaultProvider} / ${defaultModel}` : "";
    ...
    <div className="flex h-full min-h-0 flex-col gap-3">
            ...
            <>
              ...
              >
                <div className="flex flex-col gap-1.5">
                  <FormFieldLabel htmlFor="pi-provider-id" label="Provider ID" hint="仅字母、数字、点、下划线、短横线；已保存渠道点「编辑」修改" />
              ...
              <FormSection>
                ...
                <div className="flex flex-col gap-1.5">
                  <FormFieldLabel htmlFor="pi-base-url" label="地址 *" hint="provider.baseUrl；端点模式指向本机网关" />
                  <Input
                    id="pi-base-url"
                ...
                <div className="flex flex-col gap-1.5">
                  <FormFieldLabel htmlFor="pi-api" label="API 类型" hint="决定请求协议格式" />
                  <Select value={fields.api || "__none__"} onValueChange={(value) => updateField({ api: value === "__none__" ? "" : value })}>
                    <SelectTrigger id="pi-api" className="w-full">
              ...
              <FormSection title="鉴权">
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="flex flex-col gap-1.5">
                    <FormFieldLabel htmlFor="pi-api-key" label="秘钥" hint="provider.apiKey；$ENV_VAR 读环境变量 / !command 执行命令 / 其他为字面量" />
                  ...
                  <div className="flex flex-col gap-1.5">
                    <FormFieldLabel htmlFor="pi-oauth" label="OAuth" hint='provider.oauth；目前仅支持 "radius"（Radius OAuth gateway）' />
                    <Select
                      value={fields.oauth || "__none__"}
                      onValueChange={(value) => updateField({ oauth: value === "__none__" ? "" : value })}
                    >
                      <SelectTrigger id="pi-oauth" className="w-full">
                        <SelectValue placeholder="不使用" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="__none__">不使用</SelectItem>
                        <SelectItem value="radius">radius</SelectItem>
                        {fields.oauth && fields.oauth !== "radius" && (
                          <SelectItem value={fields.oauth}>{fields.oauth}</SelectItem>
                        )}
                      </SelectContent>
                    </Select>
                  </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <KeyValueEditor
                    ...
                    valuePlaceholder="值（$ENV / !cmd / 字面量）"
                    addLabel="新增 header"
                  />
                </div>
              ...
              <FormSection
                title="默认模型"
                description="写入 settings.json 的 defaultProvider / defaultModel，「应用」后生效"
                actions={
                  <Button
                    type="button"
                    variant="outline"
          ...
          <div className="min-h-0 flex-1">
            <Suspense fallback={<EditorFallback />}>
              <JsonEditor
                ...
                fill
                highlightPatterns={rightTab === "settings" ? ["default", "defaultProvider", "defaultModel"] : []}
                onChange={(nextText) => {
      ...
      <div className="relative flex items-center justify-center gap-3 rounded-lg border border-edge bg-surface px-4 py-3">
        <span className="absolute left-4 hidden text-xs text-ink-mute md:block">
          应用后写入 models.json / settings.json
        </span>
...
function ModelAdvancedFields({
  ...
  model: PiModelFields;
  modelIndex: number;
  updateModel: (index: number, patch: Partial<PiModelFields>) => void;
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-edge bg-surface-raised/30 px-3 py-3">
      <div className="grid gap-2.5 sm:grid-cols-2">
        ...
        <MiniField label="最大输出 maxTokens">
          <Input
            ...
            onChange={(e) => updateModel(modelIndex, { maxTokens: Number(e.target.value) || 0 })}
            className="h-8"
          />
        </MiniField>
        <MiniField label="单模型 api（留空继承 provider）">
          <Input
            value={model.api}
            onChange={(e) => updateModel(modelIndex, { api: e.target.value })}
            placeholder="留空继承"
            className="h-8"
          />
        </MiniField>
        <MiniField label="单模型 baseUrl（留空继承 provider）">
          <Input
            value={model.baseUrl}
            onChange={(e) => updateModel(modelIndex, { baseUrl: e.target.value })}
            placeholder="留空继承"
            className="h-8"
          />
        </MiniField>
      ...
      <label className="flex w-fit cursor-pointer items-center gap-1.5 text-xs text-ink-secondary" title="input 含 image；text 恒开启">
        <Switch
          checked={model.input.includes("image")}
          onCheckedChange={(c) => toggleInput(modelIndex, "image", c)}
        />
        图片输入
      </label>
      ...
      <ThinkingLevelMapEditor
        ...
        onChange={(next) => updateModel(modelIndex, { thinkingLevelMap: next })}
      />
      <CostTiersEditor
        value={model.costTiers}
        onChange={(next) => updateModel(modelIndex, { costTiers: next })}
      />
      <SamplingParamsEditor
        value={model.samplingParams}
        onChange={(next) => updateModel(modelIndex, { samplingParams: next })}
      />
