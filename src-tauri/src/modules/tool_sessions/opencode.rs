...
// ponytail: 会话扫描不读 OPENCODE_DB（与 cc-switch providers/opencode.rs 对齐）。
// 若以后要支持自定义库，把 OPENCODE_DB > XDG > 默认接到这一个函数，scan/delete 共用。
fn get_opencode_db_path() -> PathBuf {
    get_opencode_base_dir().join("opencode.db")
}

fn apply_opencode_busy_timeout(conn: &Connection) -> Result<(), rusqlite::Error> {
    // OpenCode keeps this DB open while running; without a busy timeout a
    // read/write during a write fails immediately. Same 2s window as Codex.
...
fn scan_sessions_sqlite() -> Vec<SessionMeta> {
    ...
    if apply_opencode_busy_timeout(&conn).is_err() {
        return Vec::new();
    }
...
pub fn load_messages_sqlite(source: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    apply_opencode_busy_timeout(&conn)
...
pub fn delete_session_sqlite(session_id: &str, source: &str) -> Result<bool, String> {
    ...
    if deleted > 0 {
        // Same-ID legacy JSON would otherwise reappear on the next scan.
        let storage = get_opencode_data_dir();
        let message_dir = storage.join("message").join(session_id);
        let _ = delete_session(&storage, &message_dir, session_id);
    }
...
mod tests {
    ...
    #[test]
    fn parse_sqlite_source_accepts_windows_drive_colon() {
        let parsed =
            parse_sqlite_source(r"sqlite:C:\Users\foo\.local\share\opencode\opencode.db:ses_abc")
                .expect("valid windows source");
        assert_eq!(
            parsed.0,
            PathBuf::from(r"C:\Users\foo\.local\share\opencode\opencode.db")
        );
    ...
    #[test]
    #[allow(deprecated)]
    fn scan_sessions_prefers_sqlite_when_ids_conflict() {
        let _guard = opencode_env_lock().lock().expect("lock");
        let temp = tempdir().expect("tempdir");
        let original_xdg = std::env::var_os("XDG_DATA_HOME");
        std::env::set_var("XDG_DATA_HOME", temp.path());
        ...
        let session_dir = storage.join("session").join("project-a");
        std::fs::create_dir_all(&session_dir).expect("create session dir");

        std::fs::write(
            session_dir.join("ses_dup.json"),
            r#"{"id":"ses_dup","title":"From JSON","directory":"/tmp/json","time":{"created":1,"updated":2}}"#,
        )
        .expect("write duplicate json session");
        std::fs::write(
            session_dir.join("ses_json_only.json"),
            r#"{"id":"ses_json_only","title":"JSON Only","directory":"/tmp/json-only","time":{"created":3,"updated":4}}"#,
        )
        .expect("write json-only session");
        ...
        conn.execute(
            "INSERT INTO session (id, title, directory, time_created, time_updated) VALUES (?1, ?2, ?3, ?4, ?5)",
            ("ses_dup", "From SQLite", "/tmp/sqlite", 10_i64, 20_i64),
        )
        .expect("insert duplicate sqlite session");
        drop(conn);
        ...
        let dup = sessions
            ...
            .expect("duplicate id");
        assert_eq!(dup.title.as_deref(), Some("From SQLite"));
        assert!(
            dup.source_path
                .as_deref()
                .is_some_and(|p| p.starts_with("sqlite:")),
            "duplicate id should keep sqlite source, got {:?}",
            dup.source_path
        );
        let json_only = sessions
            .iter()
            .find(|s| s.session_id == "ses_json_only")
            .expect("json-only id");
        assert_eq!(json_only.title.as_deref(), Some("JSON Only"));
        assert!(json_only
            .source_path
            .as_deref()
            .is_some_and(|p| !p.starts_with("sqlite:")));
    ...
    fn delete_session_sqlite_removes_session() {
        ...
        let part_dir = storage.join("part").join("msg_1");
        let session_diff = storage.join("session_diff").join("ses_1.json");
        let session_file = session_dir.join("ses_1.json");
        let project_file = storage.join("project").join("project-a.json");
        std::fs::create_dir_all(&session_dir).expect("create session dir");
        std::fs::create_dir_all(&message_dir).expect("create message dir");
        std::fs::create_dir_all(&part_dir).expect("create part dir");
        std::fs::create_dir_all(storage.join("session_diff")).expect("create session diff dir");
        ...
        std::fs::write(
            &session_file,
            r#"{"id":"ses_1","projectID":"project-a","directory":"/tmp/project-a"}"#,
        )
        .expect("write session file");
        std::fs::write(
            message_dir.join("msg_1.json"),
            r#"{"id":"msg_1","sessionID":"ses_1","role":"user"}"#,
        )
        .expect("write message file");
        std::fs::write(
            part_dir.join("prt_1.json"),
        ...
        .expect("write part file");
        std::fs::write(&session_diff, "[]").expect("write session diff");
        std::fs::write(&project_file, r#"{"id":"project-a"}"#).expect("write project file");
