...
- 各平台 Tauri 构建依赖 — 参见 [Tauri Prerequisites](https://tauri.app/start/prerequisites/)。
...
> 本地 `pnpm tauri build` 若要生成带 updater 签名的产物，需要配置 `TAURI_SIGNING_PRIVATE_KEY` 等环境变量，详见 [自动更新](./auto-update)。
...
- 项目模块划分见 [技术架构](./architecture)。
- 发布与签名见 [自动更新](./auto-update)。
